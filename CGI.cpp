...
{
	if (dup2(pipe_stdin[STDIN_FILENO].getFd(), STDIN_FILENO) == -1) // dup2 the readend of stdin since thats needed // 
	{
		throw std::runtime_error("dup2 failed for stdin: " + std::string(strerror(errno)));
	}
	if (dup2(pipe_stdout[STDOUT_FILENO].getFd(), STDOUT_FILENO) == -1) // dup2 the write end of stdout as whatever the child needs to output goes into that area // 
	{
